...
pboot <- function(modelobj, niter=1000){
  numsims <- niter
...
gamm.fit.smooth <- function(region, dataname, smooth_var, covariates = NA, knots, set_fx = FALSE, stats_only = FALSE, 
                            mod_only = FALSE, resdata = FALSE, bootstrap=TRUE, niter=1000){
  ...
  if (mod_only==FALSE & resdata==FALSE & bootstrap==TRUE){
    bootstrap_pvalue<-pboot(gamm.model, niter) 
