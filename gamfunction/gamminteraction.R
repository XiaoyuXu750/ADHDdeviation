...
gamm.smooth.predict.covariateinteraction <- function(region, dataname, smooth_var, int_var, int_var.predict.percentile, 
                                                     covariates, knots, set_fx = FALSE, increments, stats_only=TRUE, 
                                                     if_residual=FALSE, plotdf=NA){
  #clean data
  ...
  if (is.na(covariates)){
    ...
    res.int_var <- residuals(model1$mer)
    ...
    res.region <- residuals(model2$mer)
  }else{
  ...
  PCorr_Test <- corr.test(res.region, res.int_var, method="pearson")
  ...
  if (is.na(plotdf)){
    ...
    df <- get(plotdf)
    minsmooth <- min(df[[smooth_var]])
    maxsmooth <- max(df[[smooth_var]])
  }
  ...
  stats.reults<-cbind(parcel, int_var, bootstrap_pvalue, bootstrap_chisq, gam.int.pvalue, IntpartialRsq,partialRsq, Int.F, 
                      T.disease, P.disease, bootstrap.P.disease, bootstrap.chisq.disease, correstimate, corrp, changed.range, 
