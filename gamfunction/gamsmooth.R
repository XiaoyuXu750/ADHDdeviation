...
gam.fit.smooth <- function(region, dataname, smooth_var, covariates, knots, set_fx = FALSE, stats_only = FALSE, mod_only = FALSE, iter=1000){
  ...
  if (mod_only==F){
    set.seed(1)
    anova.smooth.pvalue <- anovaPB(gam.nullmodel,gam.model, n.sim = iter,test='Chisq', ncpus=1)$`Pr(>Chi)`[2]
  }else{
