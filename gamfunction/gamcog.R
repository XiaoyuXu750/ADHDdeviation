...
gam.fit.cognition <- function(region, dataname, cognition_var, smooth_var, covariates, knots, set_fx = FALSE, stats_only = FALSE){
  ...
  if (is.numeric(cognition)){
    ...
    shapiro_test1 <- shapiro.test(res1)
    shapiro_test2 <- shapiro.test(res2)
