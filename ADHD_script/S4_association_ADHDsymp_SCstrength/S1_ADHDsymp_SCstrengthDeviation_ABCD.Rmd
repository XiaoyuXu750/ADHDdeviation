...
This script test whether deviations of SC strength associates with normalized symptoms within ADHD?
...
## Step1 Associations between deviations and symptoms
...
for (VOI in symptomvar){
  if (!file.exists(paste0(interfileFolder, '/SCDeviation_', VOI,'_Age_CV75_Yeo', Yeoresolution,'_ADHDall_controlAgeSex.rds'))){
    ...
    resultsum <- mclapply(1:element_num, function(x){
      ...
      gamresult<-gamm.smooth.predict.covariateinteraction(region=VOI, dataname, smooth_var="age", int_var=region, int_var.predict.percentile=0.1, covariates="sex+meanFD", knots=3, set_fx = T, increments=100, stats_only=T, if_residual=FALSE)
...
for (VOI in symptomvar){
  ...
  ## Edges with significant age-related deviation decrease
  
  resultsum.df.sigedge.all <- resultsum.df %>% filter(int_var %in% sigedge.all)
  ...
  if (numsig.fdr2 > 0){
    ...
    for (i in 1:numsig.fdr2){
      resultsum.df.sigedge.all.fdr <- resultsum.df.sigedge.all %>% filter(bootstrap.P.fdr < 0.05)
      region <- resultsum.df.sigedge.all.fdr$int_var[i]
      ...
      subtitle <- bquote(
        italic("T") == .(round(resultsum.df.sigedge.all.fdr$T.disease[i], 2)) * "," ~ 
          italic(P) == .(round(resultsum.df.sigedge.all.fdr$bootstrap.P.fdr[i], 3))
      )
...
symass.all <- rbind(symass.ADHD, symass.ATT) %>% filter(int_var %in% sigedge.all)
symass.all$bootstrap.P.fdr <- p.adjust(symass.all$bootstrap.P.disease, method = "fdr")

write.csv(symass.all, paste0(resultFolder, "/SCDeviation_Symptom_Age_CV75_Yeo", Yeoresolution,"_ADHDall_controlAgeSex.csv"), row.names = F)

symass.all.sig <- symass.all %>% filter(bootstrap.P.fdr < 0.05)
summary(symass.all$correstimate)
#     Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
# -0.0518566 -0.0007238  0.0250528  0.0250324  0.0423596  0.1002908  
...
for (VOI in c("cbcl_scr_dsm5_adhd_t", "cbcl_scr_syn_attention_t")){
  symass.tmp <- SC_symptom.list[[VOI]]
  
  symass.tmp <- symass.tmp %>% mutate(
    correstimate2 = case_when(
      int_var %in% sigedge.all ~ correstimate,
      .default = NA
    ))
  ...
  tiff(paste0(FigureFolder, "/SCDeviation_Symptom/ChordPlot_Corr_", VOI,".tiff"), width = 15, height = 14, units = "cm", res = 300)
  circos.clear()
