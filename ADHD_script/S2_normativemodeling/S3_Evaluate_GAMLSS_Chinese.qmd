---
title: "S6_Evaluate_GAMLSS_Chinese"
author:
  - name: Xiaoyu Xu
...
mod.training.sum <- readRDS(paste0(interfileFolder, "/GAMLSS_Yeo", Yeoresolution,".TDtraining.All.rds"))
...
The normalized quantile residuals of our normative model were also checked to evaluate the goodness of fit.
...
centile_boot <- list()
for (i in 1:1000) {
  if (file.exists(paste0(bootstrapdir, "/WB_SCmean/centile_bootstrap_", i, ".rds"))){
...
is_null <- sapply(centile_boot, is.null)
centile_boot <- centile_boot[! is_null]
...
if (! file.exists(paste0(resultFolder, "/Average_bootstrap_cv_GAMLSS.csv"))){
  Average_cv.list <- mclapply(1:element_num, function(x){
    region <- paste0("SC.", x)
  ...
  Average_cv.df <- do.call(rbind, Average_cv.list)
...
axeslabels=c("VS.P", "SM.A", "VS.C", "SM.B", "DA.A", "DA.B", "FP.C", "DM.C", "VA.A", "TP", "FP.A", "VA.B", "DM.A", "FP.B", "DM.B") 
Mat.cv <- plotmatrix(dataname="Average_cv.df", variable="V1", ds.resolution=Yeoresolution.delLM, Pvar=NA, NAcol="white", lmthr=NA, axeslabels=axeslabels, axeslabelsGap=F, linerange_frame=NA, PaletteSet=PaletteSet)
Mat.cv
