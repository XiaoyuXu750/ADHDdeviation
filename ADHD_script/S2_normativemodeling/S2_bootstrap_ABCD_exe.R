...
for (i in 1:element_num){
  SClabel <- paste0("SC.", i)
  SCdata.sum75.merge.TD <- SCdataTD %>% dplyr::select(all_of(c(SClabel, "age", "sex", "meanFD", "siteID", "scanID")))
...
# "WB_SCmean"
SCdata.sum75.merge.TD <- SCdataTD %>% dplyr::select(all_of(c("WB_SCmean", "age", "sex", "meanFD", "siteID", "scanID")))
SCdata.sum75.merge.TD$sex <- factor(SCdata.sum75.merge.TD$sex, levels=c(1,2), labels=c("M", "F"))
SCdata.sum75.merge.TD$siteID <- as.factor(SCdata.sum75.merge.TD$siteID)
