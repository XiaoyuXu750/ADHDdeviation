...
deviationdf <- readRDS(paste0(interfileFolder, '/SCdata_Yeo', Yeoresolution,'_CV75_deviations_all_across20samples.rds'))
...
SCdata.ADHD[,c("sex", "site")] <- lapply(SCdata.ADHD[,c("sex", "site")], as.factor)

n0 = nrow(SCdata.ADHD)
SCdata.ADHD <- SCdata.ADHD %>% filter(WB_SCmean>mean(WB_SCmean)-3*sd(WB_SCmean), WB_SCmean<mean(WB_SCmean)+3*sd(WB_SCmean), site %in% unique(SCdata.TD.trainset$site))
n1 = nrow(SCdata.ADHD)
...
if (! file.exists(paste0(interfileFolder, "/FirstDeirvative_GAMLSS_Yeo", Yeoresolution,".TDtraining.sum.rds"))){
  DerivativeTD.sum <- mclapply(1:element_num, function(i){
    if (i == 21){
      gam.data2 <- as.data.frame(SCdata.TD.trainset2)
      gam.data2$randomvar <- as.factor(gam.data2[["site"]])
      gam.data2 <- as.data.frame(gam.data2)
    }else{
      gam.data2 <- as.data.frame(SCdata.TD.trainset)
      gam.data2$randomvar <- as.factor(gam.data2[["site"]])
      gam.data2 <- as.data.frame(gam.data2)
    }
...
gam.data2 <- gam.data2 %>% group_by(randomvar) %>%
      ...
      ungroup()
gam.data2 <- as.data.frame(gam.data2)
n_sites <- nlevels(gam.data2$site %>% droplevels())
...
CentileADHD.P50.1 <- lapply(mod.ADHD.sum, function(x) (x$centiles_strat[[1]][2,1]+x$centiles_strat[[2]][2,1])/2)
...
lmthr <- max(abs(c(Derivative.df$DerivativeTD.avg, Derivative.df$DerivativeADHD.avg)))+1e-5 # 0.08680954
...
DerivativeTD.df.l <- DerivativeTD.df %>% pivot_longer(cols=starts_with("SC."), names_to="label_ID", values_to="derivative")

peakchangeTD <- DerivativeTD.df.l %>% group_by(label_ID) %>% 
  summarise(peakchange = mean(age[round(abs(derivative), 4) == max(round(abs(derivative), 4))]), onsetage = if (sum(derivative > 0) > 0){min(age[derivative > 0])}else{NA})
...
DerivativeADHD.df.l <- DerivativeADHD.df %>% pivot_longer(cols=starts_with("SC."), names_to="label_ID", values_to="derivative")
...
# Max >=8.99, min=-5.067
peakchangeall$SCID.x <- as.numeric(peakchangeall$SCID.x)
peakchangeall <- peakchangeall[order(peakchangeall$SCID.x), ]
PaletteSet <- list(Name="RdBu", drirection=-1, lmmin = -8.992, lmmax = 8.992, anglex=45, angley=45,
                   hjustx = 1, hjusty = 1, vjustx = 1, vjusty=0.3)
Matonset.gap <- plotmatrix(dataname="peakchangeall", variable="onset.gap", ds.resolution=Yeoresolution.delLM, Pvar=NA, NAcol="gray", lmthr=NA, axeslabels=axeslabels, axeslabelsGap=F, linerange_frame=NA, PaletteSet=PaletteSet)
Matonset.gap

ggsave(paste0(FigureFolder, "/Normative_Trajectory/Onset_gap_Mat.tiff"), Matonset.gap, height = 14.2, width = 16, units = "cm")
...
CentileP50.TD.l <- CentileP50.TD %>% pivot_longer(cols=starts_with("SC."), names_to="label_ID",values_to="fit")
CentileP50.TD.l <- CentileP50.TD.l %>% left_join(Derivative.df, join_by("label_ID"=="SClabel"))
SCrankdf <- SCrankcorr(Derivative.df, "DerivativeTD.avg", Yeoresolution.delLM, T)
SCrankdf$SClabel <- paste0("SC.", 1:element_num)
SCrankdf$DerivativeTD.avg <- NULL
...
# ADHD
CentileP50.ADHD <- do.call(rbind, lapply(mod.ADHD.sum, function(x) (x$centiles_strat[[1]][2,]+x$centiles_strat[[2]][2,])/2))
...
CentileP50.ADHD$age <- seq(min(SCdata.ADHD$age), max(SCdata.ADHD$age), length.out=1000)

CentileP50.ADHD.l <- CentileP50.ADHD %>% pivot_longer(cols=starts_with("SC."), names_to="label_ID",values_to="fit")
CentileP50.ADHD.l <- CentileP50.ADHD.l %>% left_join(Derivative.df, join_by("label_ID"=="SClabel"))
SCrankdf <- SCrankcorr(Derivative.df, "DerivativeADHD.avg", Yeoresolution.delLM, T)
SCrankdf$SClabel <- paste0("SC.", 1:element_num)
SCrankdf$DerivativeADHD.avg <- NULL
CentileP50.ADHD.l <- CentileP50.ADHD.l %>% left_join(SCrankdf, join_by("label_ID"=="SClabel"))

# ADHD trajectories
CentileP50.ADHD.l$label_ID <- factor(CentileP50.ADHD.l$label_ID, levels=SCrankdf$SClabel[order(SCrankdf$SCrank)])
...
lmthr <- max(abs(deviationSCmat.df$deviationMean))+1e-5 # 0.3625048
axeslabels=c("VS.P", "SM.A", "VS.C", "SM.B", "DA.A", "DA.B", "FP.C", "DM.C", "VA.A", "TP", "FP.A", "VA.B", "DM.A", "FP.B", "DM.B") 
axeslabelsGap=T
PaletteSet <- list(Name="RdBu", drirection=-1, lmmin = -lmthr, lmmax = lmthr, anglex=45, angley=45,hjustx = 1, hjusty = 1, vjustx = 1, vjusty=0.3)
...
# 1            15 deviationMean  0.1920764 0.03558142
...
scatter <- ggplot(data=df)+
    ...
    theme(axis.text=element_text(size=19.8, color="black"), 
        axis.title =element_text(size=19.8, color="black"),aspect.ratio = 0.95,
        plot.background=element_rect(fill="white"),
        panel.background=element_rect(fill="white"),
        plot.title = element_text(size=16, hjust = 0.5),
        legend.position = "none")
ggsave(paste0(FigureFolder, '/Normative_Trajectory/AvgDeivationADHD_SCrankcorr.tiff'),scatter, width=14.5, height =14.5, units = "cm")
