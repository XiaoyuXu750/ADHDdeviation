...
This script visualizes developmental trajectories derived from the normative models and their critical features.
Besides, this script visualizes the average deviations of both groups.
...
This step aim to fit the developmental curves for the ADHD group.
...
n0 = nrow(SCdata.ADHD)
SCdata.ADHD <- SCdata.ADHD %>% filter(WB_SCmean>mean(WB_SCmean)-3*sd(WB_SCmean), WB_SCmean<mean(WB_SCmean)+3*sd(WB_SCmean), siteID %in% unique(SCdata.TD.trainset$siteID))
n1 = nrow(SCdata.ADHD)
#boxplot(SCdata.ADHD$WB_SCmean)
print(paste(n1, "obs included,", (n0-n1), "obs were removed due to extreme deviation of WB_SCmean & not in the site list."))
...
lmthr <- max(abs(c(Derivative.df$DerivativeTD.avg, Derivative.df$DerivativeADHD.avg)))+1e-5 # 0.03660626
axeslabels=c("VS.P", "SM.A", "VS.C", "SM.B", "DA.A", "DA.B", "FP.C", "DM.C", "VA.A", "TP", "FP.A", "VA.B", "DM.A", "FP.B", "DM.B") 
axeslabelsGap=T
PaletteSet <- list(Name="RdBu", drirection=-1, lmmin = -lmthr, lmmax = lmthr, anglex=0, angley=0,
...
ggsave(paste0(FigureFolder, "/Normative_Trajectory/AvgDerivative_TD_Mat.tiff"), AvgDerivativeTD, height = 14, width = 16.5, units = "cm")
ggsave(paste0(FigureFolder, "/Normative_Trajectory/AvgDerivative_ADHD_Mat.tiff"), AvgDerivativeADHD, height = 14, width = 16.5, units = "cm")
...
PaletteSet <- list(Name="BuPu", drirection=1, lmmin = lmmin, lmmax = 15.5, anglex=0, angley=0,
                   hjustx = 1, hjusty = 1, vjustx = 0.5, vjusty=0.5)
...
# Onset age gap
peakchangeall <- merge(peakchangeTD, peakchangeADHD, by="label_ID")
peakchangeall <- peakchangeall %>% mutate(
  onset.gap = case_when(is.na(onsetage.y) & !is.na(onsetage.x) ~ (15.5-onsetage.x),
                        is.na(onsetage.y) & is.na(onsetage.x) ~ NA,
                        .default = onsetage.y - onsetage.x)
)
# Max >=6.583, min=-2.076
peakchangeall <- peakchangeall[order(peakchangeall$SCID.x), ]
peakchangeall <- peakchangeall %>% mutate(
  NAlabel = case_when(is.na(onset.gap) ~ 0.001,
                      .default = 0.9)
)

PaletteSet <- list(Name="RdBu", drirection=-1, lmmin = -6.59, lmmax = 6.59, anglex=0, angley=0,
                   hjustx = 1, hjusty = 1, vjustx = 0.5, vjusty=0.5)
Matonset.gap <- plotmatrix(dataname="peakchangeall", variable="onset.gap", ds.resolution=Yeoresolution.delLM, Pvar=NA, NAcol="gray", lmthr=NA, axeslabels=axeslabels, axeslabelsGap=F, linerange_frame=NA, PaletteSet=PaletteSet, Pvar.noFDR="NAlabel")
Matonset.gap

ggsave(paste0(FigureFolder, "/Normative_Trajectory/Onset_gap_Mat.tiff"), Matonset.gap, height = 14, width = 16, units = "cm")
SCrankcorr(peakchangeall, "onset.gap", Yeoresolution.delLM)
#   ds.resolution Interest.var r.spearman   p.spearman
# 1            15    onset.gap  0.3859273 2.047686e-05

matindex <- matrix(NA, 15, 15)
matindex[lower.tri(matindex, diag = T)] <- 1:element_num
Type.S <- as.numeric(matindex[1:4, 1:4])
Type.SA <- as.numeric(matindex[5:15, 1:4])
Type.A <- as.numeric(matindex[5:15, 5:15])

Type.S <- Type.S[! is.na(Type.S)]
Type.SA <- Type.SA[! is.na(Type.SA)]
Type.A <- Type.A[! is.na(Type.A)]

peakchangeall$Type <- NA
peakchangeall$Type[Type.S] <- "SS"
peakchangeall$Type[Type.A] <- "AA"
peakchangeall$Type[Type.SA] <- "SA"

peakchangeall.l <- peakchangeall %>% pivot_longer(
  cols = c(onsetage.x, onsetage.y),
  names_to = "group",
  values_to = "onsetage"
)%>%
  mutate(group = ifelse(group == "onsetage.x", "TD", "ADHD"))

# T test for three type
resultdf <- data.frame(Type=c("SS", "AA", "SA"), Tvalue=rep(NA, 3),df=rep(NA, 3), Pvalue=rep(NA, 3), cohend=rep(NA, 3))
n=0
for (t in c("SS", "SA", "SA")){
  df.tmp <- peakchangeall.l %>% filter(Type == t)
  df.tmp$onsetage[is.na(df.tmp$onsetage)] <- 15.5
  ttest.res <- t.test(onsetage ~ group, data=df.tmp, paired = T)
  df <- ttest.res$parameter
  difference <- df.tmp %>% group_by(label_ID) %>% summarise(diff = onsetage[group=="ADHD"]-onsetage[group=="TD"])
  cohend.tmp <- mean(difference$diff) / sd(difference$diff) 
  n=n+1
  resultdf$Tvalue[n] <- ttest.res$statistic
  resultdf$df[n] <- as.numeric(df)
  resultdf$Pvalue[n] <- ttest.res$p.value
  resultdf$cohend[n] <- cohend.tmp
}

print(resultdf)

peakchangeall.l$onsetage[is.na(peakchangeall.l$onsetage)] <- 15.5
stats_df <- peakchangeall.l %>% group_by(Type, group) %>%
  summarise(
    mean_age = mean(onsetage, na.rm = TRUE),
    sd_age = sd(onsetage, na.rm = TRUE),
    .groups = "drop"
  )

stats_df$group <- factor(stats_df$group, levels = c("TD", "ADHD"))
stats_df$Type <- factor(stats_df$Type, levels = c("SS", "SA", "AA"))
barplot <- ggplot(stats_df, aes(x = Type, y = mean_age, fill = group)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_errorbar(
    aes(ymin = mean_age - sd_age, ymax = mean_age + sd_age),
...
ggsave(paste0(FigureFolder, "/Normative_Trajectory/P50fitlines_ADHD.tiff"), ADHDline, width=19, height =18, units = "cm")
...
deviationSCmat.mean <- base::colMeans(deviationSCmat)
...
lmthr <- max(abs(deviationSCmat.df$deviationMean))+1e-5 # 0.1312783
axeslabels=c("VS.P", "SM.A", "VS.C", "SM.B", "DA.A", "DA.B", "FP.C", "DM.C", "VA.A", "TP", "FP.A", "VA.B", "DM.A", "FP.B", "DM.B") 
axeslabelsGap=T
...
#   ds.resolution  Interest.var r.spearman   p.spearman
# 1            15 deviationMean 0.4364526   6.251687e-07	
...
scatter <- ggplot(data=deviationSCmat.all)+
    ...
    theme(axis.text=element_text(size=20.3, color="black"), 
        axis.title =element_text(size=20.3, color="black"),aspect.ratio = 1.07,
        plot.background=element_rect(fill="white"),
        panel.background=element_rect(fill="white"),
        plot.title = element_text(size=20, hjust = 0.5),
        legend.text = element_text(size=20, color="black"), legend.title = element_text(size=20, color="black"))
ggsave(paste0(FigureFolder, '/Normative_Trajectory/AvgDeivation_SCrankcorr_Interaction.tiff'),scatter, width=18.1, height =16.5, units = "cm")
...
Figtmp <- ggplot()+
  #geom_hex(data=pointdf.TDtest, aes(x=age, y=TDtest.standard), bins=15)+
  geom_density_2d_filled(data=pointdf.TDtest, aes(x=age, y=TDtest.standard),contour_var = "count",bins=50, alpha=1, show.legend=F)+
  scale_fill_manual(values=COL1(sequential="Greys", n=55)[c(1,6:55)])+
  #scale_fill_gradient(low = "grey90",  high = "grey20", na.value = "white") +
  #scale_color_distiller(type="seq", palette = "RdGy", direction = -1)+
  geom_line(data=pointdf.ADHD, aes(x=age, y=ADHD.standard, group=subID), color="#FDDBC7")+
  ...
  theme_classic()+
  theme(axis.text=element_text(size=20.3, color="black"), 
        axis.title =element_text(size=20.3, color="black"),aspect.ratio = 0.99,
        plot.background=element_rect(fill="white"),
        panel.background=element_rect(fill="white"),
        plot.title = element_text(size=20, hjust = 0.5),
        legend.text = element_text(size=20, color="black"), legend.title = element_text(size=20, color="black"))

ggsave(paste0(FigureFolder, "/Normative_Trajectory/NormCurve_", SClabel,"_ADHDDeviation.tiff"), Figtmp, width=19, height =17, units = "cm")



## Draw a schema

pointdf.TDtest2 <- deviationdf %>% filter(if_TD=="TD", abs(WB_SCmean_deviationZ) <2.3) 
pointdf.TDtest2 <- pointdf.TDtest2[sample(1:100, 100, F),]

pointdf.ADHDtest2 <- deviationdf %>% filter(if_TD=="ADHD") 
pointdf.ADHDtest3 <- deviationdf %>% filter(if_TD=="ADHD", WB_SCmean_deviationZ > 1.96) 
pointdf.ADHDtest2 <- pointdf.ADHDtest2[sample(1:90, 90, F),]
pointdf.ADHDtest2 <- rbind(pointdf.ADHDtest2, pointdf.ADHDtest3[sample(1:10, 10, F),])

Figtmp <- ggplot()+
  geom_line(data=pointdf.TDtest2, aes(x=age, y=WB_SCmean_standard, group=subID), color="#D9D9D9")+
  geom_point(data=pointdf.TDtest2, aes(x=age, y=WB_SCmean_standard), color="#969696", size=1.8)+
  geom_line(data=pointdf.ADHDtest2, aes(x=age, y=WB_SCmean_standard, group=subID), color="#FDDBC7")+
  geom_point(data=pointdf.ADHDtest2, aes(x=age, y=WB_SCmean_standard), color="#B2182B", size=1.8)+
  geom_line(data=centile.tmp, aes(x=age, y=fit, group=Percentile, linetype=Percentile), color="black", linewidth=0.8)+
  scale_linetype_manual(values=c("dashed", "solid", "dashed"))+
  #scale_y_continuous(breaks = c(0,1,2,3,4))+
  labs(x="Age (years)", y="SC strength")+
  theme_classic()+
  theme(axis.text=element_text(size=20.3, color="black"), 
        axis.title =element_text(size=20.3, color="black"),aspect.ratio = 0.88,
        plot.background=element_rect(fill="white"),
        panel.background=element_rect(fill="white"),
        plot.title = element_text(size=20, hjust = 0.5),
        legend.text = element_text(size=20, color="black"), legend.title = element_text(size=20, color="black"))

ggsave(paste0(FigureFolder, "/Normative_Trajectory/NormCurve_schema.tiff"), Figtmp, width=14, height =13, units = "cm")
ggsave(paste0(FigureFolder, "/Normative_Trajectory/NormCurve_schema.svg"), Figtmp, width=14, height =13, units = "cm")

### SC deviation examples
## Randomly select 3 subjects
# ADHD
mat1 <- deviationdf.ADHD %>% filter(round(age) == 9) %>% dplyr::select(ends_with("_deviationZ") & starts_with("SC."))
mat1 <- mat1[28,]
mat2 <- deviationdf.ADHD %>% filter(round(age) == 9) %>% dplyr::select(ends_with("_deviationZ") & starts_with("SC."))
mat2 <- mat2[56,]
mat3 <- deviationdf.ADHD %>% filter(round(age) == 10) %>% dplyr::select(ends_with("_deviationZ") & starts_with("SC."))
mat3 <- mat3[28,]

df <- data.frame(mat1=as.numeric(mat1), mat2=as.numeric(mat2),mat3=as.numeric(mat3))

maxvalue <- max(c(df$mat1, df$mat2, df$mat3))

PaletteSet <- list(Name="RdBu", drirection=-1, lmmin = -maxvalue, lmmax = maxvalue, anglex=0, angley=0,
                   hjustx = 1, hjusty = 1, vjustx = 1, vjusty=0.5)
axeslabels=c("VS.P", "SM.A", "VS.C", "SM.B", "DA.A", "DA.B", "FP.C", "DM.C", "VA.A", "TP", "FP.A", "VA.B", "DM.A", "FP.B", "DM.B") 

plotmat1 <- plotmatrix("df", variable="mat1", ds.resolution=15, Pvar=NA, NAcol="white", lmthr=NA, 
                       axeslabels=axeslabels, axeslabelsGap=F, linerange_frame=NA, PaletteSet=PaletteSet, Pvar.noFDR=NA)

plotmat2 <- plotmatrix("df", variable="mat2", ds.resolution=15, Pvar=NA, NAcol="white", lmthr=NA, 
                       axeslabels=axeslabels, axeslabelsGap=F, linerange_frame=NA, PaletteSet=PaletteSet, Pvar.noFDR=NA)

plotmat3 <- plotmatrix("df", variable="mat3", ds.resolution=15, Pvar=NA, NAcol="white", lmthr=NA, 
                       axeslabels=NULL, axeslabelsGap=F, linerange_frame=NA, PaletteSet=PaletteSet, Pvar.noFDR=NA)


ggsave(paste0(FigureFolder, "/Normative_Trajectory/SCdeviationADHD1.tiff"), plotmat1, height = 12.5, width = 14, units = "cm")
ggsave(paste0(FigureFolder, "/Normative_Trajectory/SCdeviationADHD2.tiff"), plotmat2, height = 12.5, width = 14, units = "cm")
ggsave(paste0(FigureFolder, "/Normative_Trajectory/SCdeviationADHD3.tiff"), plotmat3, height = 12.5, width = 14, units = "cm")

# TD
mat1 <- deviationdf.TD %>% filter(round(age) == 9, abs(WB_SCmean_deviationZ) <1.96) %>% dplyr::select(ends_with("_deviationZ") & starts_with("SC."))
mat1 <- mat1[28,]
mat2 <- deviationdf.TD %>% filter(round(age) == 9, abs(WB_SCmean_deviationZ) <1.96) %>% dplyr::select(ends_with("_deviationZ") & starts_with("SC."))
mat2 <- mat2[56,]
mat3 <- deviationdf.TD %>% filter(round(age) == 10, abs(WB_SCmean_deviationZ) <1.96) %>% dplyr::select(ends_with("_deviationZ") & starts_with("SC."))
mat3 <- mat3[28,]
...
ggsave(paste0(FigureFolder, "/Normative_Trajectory/SCdeviationTD1.tiff"), plotmat1, height = 12.5, width = 14, units = "cm")
ggsave(paste0(FigureFolder, "/Normative_Trajectory/SCdeviationTD2.tiff"), plotmat2, height = 12.5, width = 14, units = "cm")
ggsave(paste0(FigureFolder, "/Normative_Trajectory/SCdeviationTD3.tiff"), plotmat3, height = 12.5, width = 14, units = "cm")
