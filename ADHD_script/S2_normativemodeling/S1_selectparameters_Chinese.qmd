...
SCdata.TD.training <- SCdata.TD.training %>% mutate(WB_SCmean = rowMeans(dplyr::select(.,starts_with("SC."))))
...
SCdata.TD.trainset2 <- SCdata.TD.trainset %>% filter(SC.21 > mean(SC.21)-3*sd(SC.21), SC.21 < mean(SC.21)+3*sd(SC.21))
if (! file.exists(paste0(resultFolder, "/distributiontest_TD_Edgelevel_Yeo", Yeoresolution,".rds"))){
  ...
  for (i in 1:element_num){
    ...
    if (i == 21){
      outlist.tmp <- gamlss_comparedistribution("SCdata.TD.trainset", dependentvar, smoothvar,IDvar, bs.df, covariates, randomvar, cl)
    }else{
      outlist.tmp <- gamlss_comparedistribution(dataname, dependentvar, smoothvar,IDvar, bs.df, covariates, randomvar, cl)
    }
    performance.tmp <- outlist.tmp$performance
