# This function is designed to fit a GAMLSS model on a bootstrap-resampled dataset.
...
execute_boot <- function(n, var){
  ...
  if (var == "SC.21"){
    SCdataTD$tmp <- SCdataTD[[var]]
    SCdataTD <- SCdataTD %>% filter(tmp > mean(tmp)-3*sd(tmp), tmp < mean(tmp) + 3*sd(tmp))
  ...
  SCdata.sum75.merge.TD <- SCdataTD %>% dplyr::select(c(var, "age", "sex", "mean_fd", "site", "ID"))
