...
This script performs parameter selection for normative models.
...
SCdata.TD <- SCdata.TD %>% mutate(WB_SCmean = rowMeans(dplyr::select(.,starts_with("SC."))))
n0 = nrow(SCdata.TD)
...
##########################################
# Split the TD dataset into two subsets:
#   (1) single-visit subjects and (2) multi-visit subjects (>1 visit).
# Perform stratified sampling within each subset to obtain 70% training and 30% test sets.
# For the single-visit subset, stratify by sex, siteID, and eventname.
# For the longitudinal subset, stratify by sex and siteID.
...
## (2) multi-visit subjects (>1 visit)
...
Select the best degree of freedom (2–6) and polynomial degree (2–3).
...
# All edges
performance.df.sum$dfset <- gsub("mu", "μ", performance.df.sum$dfset)
performance.df.sum$dfset <- gsub("sigma", "σ", performance.df.sum$dfset)
performance.df.sum$dfset <- gsub("degree", "deg", performance.df.sum$dfset)
...
performance.df.sum <- performance.df.sum %>% filter(dfset %in% levels(performance.df.sum$dfset)[1:5])

ggplot(data=performance.df.sum)+
  geom_bar(aes(x=dfset, y=bestnum),stat="identity", fill="#B4D3E7")+
  labs(x=NULL, y="Frequency")+theme_classic()+
  theme(panel.background = element_rect(fill="white"),
        plot.background = element_rect(fill = "white",colour = NA),aspect.ratio = 0.7,
        plot.title = element_text(color = "black", size = 14, hjust = 0.5),
        axis.title = element_text(color = "black", size = 14),axis.line = element_line(linewidth = 0.4),
