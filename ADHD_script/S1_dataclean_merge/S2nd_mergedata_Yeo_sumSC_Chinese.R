...
Volume_path_EFNYnoCCNP <-'/ibmgpfs/cuizaixu_lab/congjing/brainproject/development/results/schaefer400_nodevolume'
...
interfileFolder_PKU6 <- paste0(homepath, '/Normative_model/interfileFolder_PKU6')
...
if (str_detect(wdpath, "cuizaixu_lab")){
  SCdata.sum <- mclapply(1:nrow(Behavior), function(i){
    ...
    if (site == "EFNY"){
      ...
      SC_file_path <- paste0(SC_path_EFNYnoCCNP, '/', SCname)
      volumefile <- paste0(Volume_path_EFNYnoCCNP, '/', ID, '_Volume7.txt')
    }else if (site == "PKU6"){
  ...
  SCdata.sum75.merge <- merge(SCdata.sum75, Behavior, by="ID")
