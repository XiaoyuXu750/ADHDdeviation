...
Script for inclusion and exclusion of samples in the EFNY study.
...
basic_demoCJ <- basic_demoCJ %>% left_join(dplyr::select(MRItable, ID, Name), by="ID")
...
MRItable %>% filter(Name %in% basic_demo.df.other$Name[abnormalID]) %>% dplyr::select("Name", "ID")
...
basic_demo.df.other <- basic_demo.df.other %>% left_join(dplyr::select(MRItable, ID, Name, Age, Sex, subID), by="Name")
...
basic_demo.df4 <- basic_demo.df4 %>%
  left_join(ADHD_df %>% dplyr::select(subID, ADHD), by = "subID")

abnormalID <- which(is.na(basic_demo.df4$ADHD))
basic_demo.df4[abnormalID, c("subID", "Name", "ID")]
basic_demo.df4[basic_demo.df4$ID == 276, "ADHD"] <- 1
basic_demo.df4[basic_demo.df4$ID == "LYJ", "ADHD"] <- 0

table(basic_demo.df4$ADHD)
```
...
basic_demo.df5 <- basic_demo.df4 %>% left_join(dplyr::select(basic_demo.df, Hear_problems, Colour_blindness, ID), by="ID") %>% distinct(ID, .keep_all = T)
...
table(basic_demo.df5$ADHD)

```
### 4) Intellectual disability

Only participants with IQ over 80 were included.
