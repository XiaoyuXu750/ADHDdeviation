...
Script for inclusion and exclusion of samples in the ABCD study.
...
demo_sublist0 <- demo_sublist0 %>% left_join(dplyr::select(ADHD_l_commor, scanID, exclude_commorbid1, exclude_commorbid2), by="scanID")
...
demo_sublist0 <- merge(demo_sublist0, filerecord_all, by="scanID", all.x = T)

## 1. complete raw data
demo_sublist <- demo_sublist0 %>% filter(dMRI_size>0 & T1WI_size>0 & fmap_size>0)
# Philips dMRI with less than 102 gradients
index_P_incomplete <- which(demo_sublist$scanner=="Philips" & demo_sublist$philips_gradientnum<102)
demo_sublist <- demo_sublist[-index_P_incomplete, ]
num_exclude2_base <- nrow(demo_sublist0[demo_sublist0$eventname2=="baselineYear1Arm1",]) - nrow(demo_sublist[demo_sublist$eventname2=="baselineYear1Arm1",])
num_exclude2_2year <- nrow(demo_sublist0[demo_sublist0$eventname2=="2YearFollowUpYArm1",]) - nrow(demo_sublist[demo_sublist$eventname2=="2YearFollowUpYArm1",])
num_exclude2_4year <- nrow(demo_sublist0[demo_sublist0$eventname2=="4YearFollowUpYArm1",]) - nrow(demo_sublist[demo_sublist$eventname2=="4YearFollowUpYArm1",])

tab <- table(demo_sublist$if_TD, demo_sublist$eventname2)

for (i in 1){
  print(paste("2nd,", nrow(demo_sublist), "have complete MRI data via fasttrack,", num_exclude2_base, "participants were excluded from baseline,", num_exclude2_2year, "participants were excluded from 2year-follow-up,", num_exclude2_4year, "participants were excluded from 4year-follow-up."))
...
## Step3 general exclusion by demographic information
...
demo_sublist1 <- demo_sublist %>% left_join(demo_sublist_criteria, by="scanID")

demo_sublist1 <- demo_sublist1 %>% 
...
num_exclude1_base <- sum(demo_sublist$eventname2=="baselineYear1Arm1") - sum(demo_sublist1$eventname=="baseline_year_1_arm_1")
num_exclude1_2year <- sum(demo_sublist$eventname2=="2YearFollowUpYArm1") - sum(demo_sublist1$eventname=="2_year_follow_up_y_arm_1")
num_exclude1_4year <- sum(demo_sublist$eventname2=="4YearFollowUpYArm1") - sum(demo_sublist1$eventname=="4_year_follow_up_y_arm_1")
num_exclude1 = num_exclude1_base+num_exclude1_2year+num_exclude1_4year

tab <- table(demo_sublist1$if_TD, demo_sublist1$eventname2)
...
## Step4 MRI & commorbidity

1) successfully converted to BIDS.
2) passed official QC.
3) without commorbidity of schizophrenia (present or past), major depression (present), substance abuse.

```{r step4_completeMRI}

## 1. successful unzip & dcm2nii
demo_sublist3 <- demo_sublist1 %>% filter(if_bids==T)
num_exclude3_base <- nrow(demo_sublist1[demo_sublist1$eventname2=="baselineYear1Arm1",]) - nrow(demo_sublist3[demo_sublist3$eventname2=="baselineYear1Arm1",])
num_exclude3_2year <- nrow(demo_sublist1[demo_sublist1$eventname2=="2YearFollowUpYArm1",]) - nrow(demo_sublist3[demo_sublist3$eventname2=="2YearFollowUpYArm1",])
num_exclude3_4year <- nrow(demo_sublist1[demo_sublist1$eventname2=="4YearFollowUpYArm1",]) - nrow(demo_sublist3[demo_sublist3$eventname2=="4YearFollowUpYArm1",])
...
## Step5 merge demographic information


```{r merge_demo}
demo_sublist6 <- demo_sublist6 %>% dplyr::select(!(contains("size") | contains("num") | contains("include") | contains("nii")))
...
demo_sublist6 <- demo_sublist6 %>% left_join(dplyr::select(abcd_p_demo, c("src_subject_id", "demo_sex_v2")), by="src_subject_id", multiple="first")

# handedness
nc_y_ehis <- read.csv(paste0(release5path, '/neurocognition/nc_y_ehis.csv'))
demo_sublist6 <- demo_sublist6 %>% left_join(dplyr::select(nc_y_ehis, c("src_subject_id", "ehi_y_ss_scoreb")), join_by("src_subject_id"), multiple="first")
...
demo_sublist6 <- demo_sublist6 %>% left_join(dplyr::select(nc_y_nihtb, all_of(interest_cogvar)), by=join_by(src_subject_id, eventname))
...
mh_p_cbcl <- mh_p_cbcl %>% dplyr::select(c("src_subject_id", "eventname", ends_with("_r"), ends_with("_t")))
demo_sublist7 <- merge(demo_sublist7, mh_p_cbcl, by=c("src_subject_id", "eventname"), all.x=T)


# SES
abcd_p_demo <- read.csv(paste0(release5path,'/abcd-general/abcd_p_demo.csv'))
demo_sublist7 <- demo_sublist7 %>% left_join(dplyr::select(abcd_p_demo, c("src_subject_id", "eventname", "demo_comb_income_v2", "demo_roster_v2", "demo_comb_income_v2_l", "demo_roster_v2_l")), join_by("src_subject_id", "eventname"))
...
demo_sublist7 <- demo_sublist7 %>% left_join(dplyr::select(abcd_p_demo, c("src_subject_id", "eventname", "race_ethnicity")), join_by("src_subject_id", "eventname"))
...
pFactor <- pFactor %>% left_join(dplyr::select(abcd_p_demo, src_subject_id, demo_sex_v2), join_by(srcsubjectid==src_subject_id), multiple="first")
...
pFactor <- pFactor %>% left_join(dplyr::select(abcd_y_lt, src_subject_id, eventname2, interview_age), join_by(srcsubjectid==src_subject_id, eventname2==eventname2))
pFactor$demo_sex_v2 <- as.factor(pFactor$demo_sex_v2)
pFactor$interview_age <- as.numeric(pFactor$interview_age)

ADHD2dim <- ADHD2dim %>% left_join(dplyr::select(abcd_p_demo, src_subject_id, demo_sex_v2), join_by(srcsubjectid==src_subject_id), multiple="first")
ADHD2dim <- ADHD2dim %>% left_join(dplyr::select(abcd_y_lt, src_subject_id, eventname2, interview_age), join_by(srcsubjectid==src_subject_id, eventname2==eventname2))
...
demo_sublist7 <- demo_sublist7 %>% left_join(dplyr::select(pFactor, srcsubjectid, eventname2, GENERAL.norm, EXT.norm, ADHD.norm, INT.norm), join_by(src_subject_id==srcsubjectid, eventname2==eventname2))
demo_sublist7 <- demo_sublist7 %>% left_join(dplyr::select(ADHD2dim, srcsubjectid, eventname2, ATT.norm, HI.norm), join_by(src_subject_id==srcsubjectid, eventname2==eventname2))
