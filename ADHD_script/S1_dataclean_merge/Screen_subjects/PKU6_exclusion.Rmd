...
Script for inclusion and exclusion of samples in the PKU6 study.
...
library(readxl)
library(stringr)
result_path <- "D:/xuxiaoyu/open_dataset_information/PKU6"
demo_path <- "D:/xuxiaoyu/open_dataset_information/PKU6/demography"
...
basic_demo.df <- basic_demo.df %>% dplyr::select(-`age(m)`, -`age (month)`)
...
## Commorbidity
ASDlist <- read.xlsx(paste0(demo_path, "/ASD_id.xlsx"))
ASDlist$ID2 <- str_split_i(ASDlist$ID, "&", 1)

## Exclude commorbid ASD
basic_demo.df <- basic_demo.df %>% filter(! ID %in% ASDlist$ID2) # N=666
...
print(paste("filter subject without age and sex: ", nrow(basic_demo.df))) # 555
table(basic_demo.df$ADHD)
basic_demo.df <- basic_demo.df %>% filter(age >= 6.5, age <= 15.5)

print(paste("filter subject without age and sex: ", nrow(basic_demo.df))) # 544
table(basic_demo.df$ADHD)
...
print(paste("filter subject with fd: ", nrow(basic_demo.df))) # 550
table(basic_demo.df$ADHD)
...
basic_demo.df <- basic_demo.df %>% dplyr::select(-`IA-3`, -`HI-3`, -`TO-3`)
