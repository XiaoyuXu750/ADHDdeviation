...
# plot SC matrix for the schematic.
## Select a subject randomly
SCdata.tmp1 <- as.numeric(SCdata.sum75.merge[12, paste0("SC.", 1:elementnum)])
SCdata.tmp2 <- as.numeric(SCdata.sum75.merge[777, paste0("SC.", 1:elementnum)])
SCdata.tmp3 <- as.numeric(SCdata.sum75.merge[1000, paste0("SC.", 1:elementnum)])

df <- data.frame(SCstrength1=SCdata.tmp1, SCstrength2=SCdata.tmp2, SCstrength3=SCdata.tmp3)
# 0~4.83
PaletteSet <- list(Name="Blues", drirection=1, lmmin = min(c(df$SCstrength1, df$SCstrength2, df$SCstrength3)), 
                   lmmax = max(c(df$SCstrength1, df$SCstrength2, df$SCstrength3)), 
                   anglex=0, angley=0,hjustx = 1, hjusty = 1, vjustx = 1, vjusty=0.5)
axeslabels=c("VS.P", "SM.A", "VS.C", "SM.B", "DA.A", "DA.B", "FP.C", "DM.C", "VA.A", "TP", "FP.A", "VA.B", "DM.A", "FP.B", "DM.B") 

for (i in c(1:3)){
  if (i ==1){label=axeslabels}else{label=NULL}
  
  
  SCmatPlot <- plotmatrix(dataname="df", variable=paste0("SCstrength",i), 
                          ds.resolution=Yeoresolution.delLM, Pvar=NA, NAcol="white", 
                          lmthr=NA, axeslabels=label, axeslabelsGap=F, 
                          linerange_frame=NA, PaletteSet=PaletteSet, Pvar.noFDR=NA)
  SCmatPlot
  
  ggsave(paste0(FigureFolder,"/CV75/Matrix_Yeo",Yeoresolution, "_sumSCinvnode_Age8_22/Random_SCmat", i, ".tiff"), 
         SCmatPlot, height = 13, width = 15, units = "cm")
