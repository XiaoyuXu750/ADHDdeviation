...
SC deviations ~ s(age, k=3, fx=T) + meanFD + sex + (1 | subID)
...
## Save out effect size table
sigedge.dev <- gamresultsum.df.ADHD.SC$parcel[gamresultsum.df.ADHD.SC$bootstrap_p.fdr<0.05]
df <- data.frame(parcel=sigedge.dev, Label=rep(NA, length(sigedge.dev)), F.TD=rep(NA, length(sigedge.dev)),F.ADHD=rep(NA, length(sigedge.dev)),Pfdr.TD=rep(NA, length(sigedge.dev)), Pfdr.ADHD=rep(NA, length(sigedge.dev)), Chisq.interaction=rep(NA, length(sigedge.dev)),Pfdr.interaction=rep(NA, length(sigedge.dev)))
...
write.csv(df, paste0(resultFolder, "/SCdeviationDevelopment_sig_STats.csv"), row.names = F)
...
## Models
for (dataname in dataname.list){
  if (! file.exists(paste0(interfileFolder, '/gammodelAllSCDeviation_NormSymptom_Age_Yeo', Yeoresolution,'_CV75_',str_split_i(dataname, "SCdata.", 2),'.rds'))){
    resultsum <- list()
...
for (x in 1:length(c(sigedge.all, symptomvar))){
  ...
  if (str_detect(region, "cbcl_")){
    PointFig <- ggplot(data=data.ADHD)+
      ...
      theme(axis.text=element_text(size=20, color="black"), 
            ...
            panel.background=element_rect(fill="transparent"),
            legend.position = 'none')
    ggsave(paste0(FigureFolder, "/SCstrength_Age/Deviationcompare/DeviationChange/ADHD_", region, "_Point.tiff"), PointFig, width=11.8, height = 11.8, units = "cm")
  }else{
    PointFig <- ggplot(data=data.ADHD)+
      geom_line(data=SCdata.ADHD, aes(x=age, y=Deviation, group=subID), color="gray", alpha=0.3)+
      geom_point(data=SCdata.ADHD, aes(x=age, y=Deviation), color= "#810F7C", alpha=0.3)+
...
ggsave(paste0(FigureFolder, "/SCstrength_Age/Deviationcompare/Matrix", Yeoresolution.delLM,"_DeviationAge_Rsq_ADHD_addintP.tiff"),FigintR2, height=14, width=16, units = "cm")
