...
plotmatrix <- function(dataname, variable, ds.resolution, Pvar=NA, NAcol="white", lmthr=NA, 
  ...
  if (is.na(Pvar.noFDR)==0){
    ...
    Matrix.tmp.sig_noFDR[lower.tri(Matrix.tmp.sig_noFDR, diag = T)] <- (data.tmp[[Pvar.noFDR]] <0.05)
  ...
  if (is.na(lmthr)){
    lmthr <- max(abs(data.tmp[[variable]]), na.rm=T)
  }
  ...
  if (is.null(axeslabels)){
    ...
    axeslabels.x <- NULL
    axeslabels.y <- axeslabels[abs(axesbreaksy)]
  }else{
    ...
    axeslabels.x <- axeslabels[abs(axesbreaks)]
    axeslabels.y <- axeslabels[abs(axesbreaksy)]
  }
  ...
  if (length(PaletteSet) == 1){
    ...
    PaletteSet$angley = 0
    PaletteSet$hjustx = PaletteSet$hjusty = 1
    PaletteSet$vjustx = PaletteSet$vjusty = 0.5
  }
  ...
  Fig <- ggplot(data=matrixtmp.df.melt)+
    ...
    ggtitle(label = plottitle)+labs(x=NULL, y=NULL, color=NULL, fill=NULL)+
    scale_y_continuous(breaks=axesbreaksy, labels = axeslabels.y, expand = c(0, 0))+
    scale_x_continuous(breaks=axesbreaks, labels = axeslabels.x, expand = c(0, 0), position = "bottom")+
    theme(axis.line = element_blank(), 
          #axis.ticks=element_line(linewidth = 0),
          axis.text.x=element_text(size=18, angle=PaletteSet$anglex, hjust=PaletteSet$hjustx, vjust=PaletteSet$vjustx, color="black"), 
          axis.text.y=element_text(size=18, angle=PaletteSet$angley, hjust=PaletteSet$hjusty,vjust=PaletteSet$vjusty, color="black"),
