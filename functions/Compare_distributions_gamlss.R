...
gamlss_comparedistribution <- function(dataname, dependentvar, smoothvar,IDvar, bs.df, covariates, randomvar=NA, cl){
  ...
  gam.data <- gam.data %>% drop_na(c(dependentvar, smoothvar))
  gam.data$age <- gam.data[[smoothvar]]
  
  gam.data$tmp <- gam.data[[dependentvar]]
  gam.data <- gam.data %>% filter(tmp > mean(tmp)-3*sd(tmp), tmp < mean(tmp) + 3*sd(tmp))
  
  
  covariates <- gsub(" ", "", covariates)
  if (! is.na(randomvar)){
    gam.data1 <- gam.data %>% dplyr::select(c(dependentvar, "age", unlist(strsplit(covariates, "+", fixed=T)), randomvar, IDvar)) %>% drop_na()
    ...
    gam.data2 <- gam.data %>% dplyr::select(c(dependentvar, "age", unlist(strsplit(covariates, "+", fixed=T)), IDvar)) %>% drop_na()
  ...
  familylist <- c("JSU", "BCCG", "BCPE", "BCPEo", "BCT", "exGAUS", "EGB2", 
...
gamlss_compare.bs.df <- function(dataname, dependentvar, smoothvar, IDvar, bs.df.set, covariates, distribution.fam, randomvar=NA, cl){
  ...
  if (! is.na(randomvar)){
    ...
    gam.data2 <- gam.data %>% dplyr::select(c(dependentvar, "age", unlist(strsplit(covariates, "+", fixed=T)))) %>% drop_na()
  }
    ...
    tryCatch(
      {
        ...
        command <- paste0("mod.tmp <- gamlss(mod.mu.formula, sigma.formula =~ bs(age, df = ", sigma.df, ", degree = ", degree,") + ", 
                          covariates, 
                          ", nu.formula = ~1, family=", distribution.fam,", 
                      data=gam.data2, control=con)")
...
Boot.Function <- function( n, Base.Seed, dataname,smoothvar,randomvar, model_obj, stratify=NULL ) {
  ...
  if (! is.na(randomvar)){
    gam.data1 <- gam.data %>% dplyr::select(unique(c(dependentvar, smoothvar, unlist(strsplit(covariates, "+", fixed=T)), randomvar, stratify))) %>% drop_na()
    gam.data1$dependent <- gam.data1[[dependentvar]]
    gam.data1$randomvar <- as.factor(gam.data1[[randomvar]])
    #gam.data1 <- gam.data1 %>% filter((dependent>mean(dependent)-3*sd(dependent)) & (dependent<mean(dependent)+3*sd(dependent)))
    gam.data1$dependent <- NULL
    ...
    gam.data2 <- gam.data %>% dplyr::select(c(dependentvar, "age", covariates)) %>% drop_na()
  }
