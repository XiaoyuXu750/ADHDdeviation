...
parallelmediation <- function(mediator, dataname, out_var, X_var, covariates, boottime=1000, longitudinal=T){
  ...
  LVSV_boot <- parameterEstimates(LVSV_fit, boot.ci.type = "perc", standardized=T)
  ...
  total := c '
    ...
    if (length(mediator) == length(mediator2)){
      ...
      LVSV_boot2 <- parameterEstimates(LVSV_fit2, boot.ci.type = "perc", standardized=T)
      ...
      LVSV_boot.final <- parameterEstimates(LVSV_fit.final, boot.ci.type = "perc", standardized=T)
    }
